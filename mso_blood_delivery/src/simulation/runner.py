...
_COL_ORDER = ["rep_id", "seed", "FR", "ADT", "P95", "ERR", "AWT", "deliveries_per_hour"]
...
def _run_single(
    ...
    # Expire any requests still waiting in hospital queues at end of simulation
...
class SimulationRunner:
    ...
    def run(
        ...
        """Run n_replications with the given policy using CRN seeds.

        Returns DataFrame of shape (n_replications, n_metrics).
        Columns: [rep_id, seed, FR, ADT, P95, ERR, AWT, deliveries_per_hour, ...]
        """
        if seeds is None:
        ...
        for rep_idx, seed in enumerate(seeds):
            ...
            result = _run_single(seed, self.lambda_hat, self.d_km, self.sim_cfg, policy)
