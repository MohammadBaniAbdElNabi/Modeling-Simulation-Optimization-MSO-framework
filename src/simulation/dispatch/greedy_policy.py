...
class NearestFeasibleGreedy(BaseDispatch):
    """Policy 2 — Nearest-Feasible Greedy Dispatch."""
    ...
    def dispatch(
        ...
        for req in list(pending):
            ...
            # Sort drones by battery descending so feasibility checks pass quickly.
            h = req.hospital_id
            sorted_idles = sorted(idle_drones, key=lambda d: d.battery, reverse=True)
            for bank in banks:
                b    = bank.bank_id
                d_km = float(d_matrix[b, h])
                t_est = d_km / 50.0 * 60.0 + 2.0   # approx flight + loading (min)
                if t_est >= best_time:
                    continue  # bank already farther than current best
                for drone in sorted_idles:
                    if not _feasibility_check(drone, req, bank, d_km, env_now):
                        continue
                    # First feasible (highest-battery) drone at this bank
                    best_time  = d_km / drone.speed_kmh * 60.0 + 2.0
                    best_drone = drone
                    best_bank  = bank
                    break
